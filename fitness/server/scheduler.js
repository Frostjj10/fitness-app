...
import { buildWorkoutFromTemplateDay, DEFAULT_TEMPLATES, CORE_EXERCISES } from '../shared/ppl.js';
...
function readUserDefaults() {
  try {
    const path = join(DATA_DIR, 'userDefaults.json');
...
function readTemplates() {
  try {
    const path = join(DATA_DIR, 'templates.json');
    if (fs.existsSync(path)) return JSON.parse(fs.readFileSync(path, 'utf-8'));
  } catch (_) {}
...
export function buildMesocycle(user, workoutDays, startDate, templateId = 'ppl') {
  const userDefaults = readUserDefaults();
